...
#define DEVICE_NAME                     "BBC 15"                               /**< Name of device. Will be included in the advertising data. */
...
char ID[]="I00015T01";
char IDandHeart[13];
int32_t IDandHeartandSpo2[18];
int32_t IDandHeartandSPo2andBat[22];
int32_t IDandHeartandSPo2andBatandTemp[26];
...
NRF_LOG_INFO("Temp >320.ON HR AGAIN");
 ...
 read_ppg();
  ...
  //HR
        ...
        //Nhietdo
           itoa(temp10, snhietdo, 10);  
      
            if( temp10==0|temp10<10)
           {
            strcpy(nhietdo,zeronhietdo2);
            strcat(nhietdo, snhietdo);
            }
            else  if( temp10>10 && temp10 <100 )
           {
            strcpy(nhietdo,zeronhietdo1);
            strcat(nhietdo, snhietdo);
            }
            else if (temp10>=100)
            {
            strcpy(nhietdo,zeronhietdo);
            strcat(nhietdo, snhietdo);
            }
          ...
          NRF_LOG_INFO("Nhiet do %s",nhietdo);
           ...
           uint16_t length = 22;
            ble_nus_data_send(&m_nus, IDandHeartandSPo2andBat, &length, m_conn_handle);
...
int main(void)
{
    ...
    //while(1){
    //    test = nrf_gpio_pin_read(30);
    //    if(test == 0){

    //      sleep_mode_enter();
    //    }
    //    NRF_LOG_INFO("test:%d",test);
    //    NRF_LOG_FLUSH();
    //    nrf_delay_ms(100);
    //}
    //if(max30208_init() == 1){
    //    NRF_LOG_INFO("enableDATARDY");
    //    NRF_LOG_FLUSH();
    //    enableDATARDY();
